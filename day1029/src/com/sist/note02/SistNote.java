...
public class SistNote extends JFrame implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(cmd.equals("새파일")) {
			JOptionPane.showConfirmDialog(this, "저장하시겠습니까?", "저장할거니", JOptionPane.YES_NO_OPTION);
			jta.setText("");
			setTitle("제목 없음");
			
			
		}else if(cmd.equals("열기")) {
			try {	//showConfirmDialog() 사용자로부터 Yes/No 응답을 입력받는 다이얼로그
			int rs = JOptionPane.showConfirmDialog(this, "저장하시겠습니까?", "저장할거니", JOptionPane.YES_NO_OPTION);
			// ok누르면 0반환 no누르면 1반환함..
			if(rs == JOptionPane.YES_OPTION) {//저장
